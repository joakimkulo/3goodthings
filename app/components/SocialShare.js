...
export default function SocialShare({ story, index, editionDate }) {
  ...
  async function downloadForInstagram() {
    setFeedback('Preparing an Instagram image…');
    try {
      const file = await getShareImage();
  ...
  return <div className="share-block">
    ...
    <div className="share-row desktop-share-row" aria-label={`Share ${story.title}`}>
      ...
      <button className="instagram-download" type="button" onClick={downloadForInstagram} aria-label="Download this story for Instagram" title="Download for Instagram">
